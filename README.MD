...
There are zero if/elif branches used. The factory does not use conditional logic to pick operators.
...
Each operator (Redact, Replace, Mask, Initial, etc.) implements the same Operator interface but defines its own operate() behavior.
The AnonymizerEngine never hard-codes any specific operator logic.
Instead, it asks the factory for an operator by name at runtime and executes its operate() method.

Because the behavior can be swapped just by changing the operator name in configuration, this is exactly the Strategy design pattern.
